...
var onCronTrigger = (runtime2) => {
  return isPositionLiquidatable(runtime2, runtime2.config.evms[0]);
};
var isPositionLiquidatable = (runtime2, evmConfig) => {
  try {
    ...
    const contractCall = evmClient.callContract(runtime2, {
      call: encodeCallMsg({
        from: zeroAddress,
        to: evmConfig.minStableVaultAddress,
        data: callData
      }),
    ...
    runtime2.log(`Liquidatable: ${liquidatable}`);
    return liquidatable;
  } catch (error) {
    runtime2.log(`Error in isPositionLiquidatable: ${error}`);
    return false;
  }
};
